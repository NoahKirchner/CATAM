...
pub struct Kernel32 {
    ...
    createthread:unsafe extern "C" fn (*const c_void, u32, *mut c_void, *const c_void, u32, *const u32)->isize,
...
impl Kernel32 {
    pub unsafe fn parse(process_header:PeHeader)->Kernel32{
        ...
        let pvirtualprotect:*mut c_void = function_table.get("VirtualProtect").expect("Failed to get function").address;
        let virtualprotect:unsafe extern "C" fn(lpaddress:*const c_void, dwsize:usize, flnewprotect:u32, lpfloldprotect:*mut u32)->() = transmute(pvirtualprotect);

        let pcreatethread:*mut c_void = function_table.get("CreateThread").expect("Failed to get function").address;
        // TODO error handling for thread return (if you really care about that lol)
        let createthread:unsafe extern "C" fn(lpthreadattributes:*const c_void, dwstacksize:u32, lpstartaddress:*mut c_void, lpparameter:*const c_void, dwcreationflags:u32, lpthreadid:*const u32)->isize = transmute(pcreatethread);

        let pwaitforsingleobject:*mut c_void = function_table.get("WaitForSingleObject").expect("Failed to get function").address;
        let waitforsingleobject:unsafe extern "C" fn(hhandle:isize, dwmilliseconds:u32)->() = transmute(pwaitforsingleobject);
    ...
    pub unsafe fn VirtualProtect(&self, lpaddress:*const c_void, dwsize:usize, flnewprotect:u32, lpfloldprotect:*mut u32)->() {
        (self.virtualprotect)(lpaddress, dwsize, flnewprotect, lpfloldprotect)
    }

    pub unsafe fn CreateThread(&self, lpthreadattributes:Option<*const c_void>, dwstacksize:u32, lpstartaddress:*mut c_void, lpparameter:Option<*const c_void>, dwcreationflags:u32, lpthreadid:Option<*const u32>)->isize {
        ...
        (self.createthread)(lp_thread_attributes, dwstacksize, lpstartaddress, lp_parameter, dwcreationflags, lp_thread_id)
