...
pub unsafe fn export_dll(dll_base_address: *mut c_void) -> HashMap<String, FunctionEntry> {
    ...
    // header structs
    let export_directory: IMAGE_EXPORT_DIRECTORY = *(dll_base_address
        .offset(nt_header.OptionalHeader.DataDirectory[EXPORT].VirtualAddress as isize)
    ...
    let name_array = slice::from_raw_parts(
        dll_base_address.offset(export_directory.AddressOfNames as u32 as isize) as *const u32,
        function_count as usize,
    );

    let address_array = slice::from_raw_parts(
        dll_base_address.offset(export_directory.AddressOfFunctions as u32 as isize) as *const u32,
        function_count as usize,
    );

    // This is u16 because it's a WORD not a DWORD.
    let ordinal_array = slice::from_raw_parts(
        dll_base_address.offset(export_directory.AddressOfNameOrdinals as u32 as isize)
            as *const u16,
        function_count as usize,
    );
    ...
    for function in 0..function_count {
        ...
        let pfunction_name: *const c_char =
            dll_base_address.offset(*name_offset as isize) as *const c_char;
        let function_name: String = CStr::from_ptr(pfunction_name)
            .to_string_lossy()
            .into_owned();
        ...
        let function_address: *mut c_void =
            dll_base_address.offset(*address_offset as isize) as *mut c_void;
